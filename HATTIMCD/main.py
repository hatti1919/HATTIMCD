...
# data.cat でストア情報を取得できるグループ (group-h が優先)
...
class DecodedOrder:
    ...
    pickup_method: str = ""   # HEXから判定した受け取り方 (日本語)
...
class OrderResult:
    ...
    group: str
    raw_hex: str
    receipt_number: str = ""   # 受け取り番号 (GetPaidOrder field[9])
    store_name: str = ""       # 店名 (data.cat から取得)
    pickup_method: str = ""    # 受け取り方 (日本語)
...
class MCD:
    ...
    def pay_from_hex(
        ...
        # 5: AuthoriseOrder (内部で root_paseto 再リフレッシュ)
        auth = self.authorise_order(so.order_token, group=so.group)

        # 6: GetPaidOrder → receipt_number
